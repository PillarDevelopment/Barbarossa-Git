...
contract MMMTokenCoin is MintableToken {
    ...
    uint256 public totalSupply;
    uint countDays;
    uint currentDate; // текущая дата
    mapping(address => uint) dateOfStart;    
    string public constant name = "Make Much Money";
    string public constant symbol = "MMM";
    uint32 public constant decimals = 2;
    uint globalInterestDate; // дата глобальной торговли

    // Crowdsale 
    uint stage2StartTime;  // дата начала 2-й стадии
   ...
   // DEBUG
    address ad1=0xCe66E79f59eafACaf4CaBaA317CaB4857487E3a1; // address 2
    address ad2=0x7eDE8260e573d3A3dDfc058f19309DF5a1f7397E; // address 3
    ...
    function MMMTokenCoin() payable  {  currentDate=(now/1 days)*1 days;
        ...
        stage2StartTime=now+2 hours; // original - now+30 days;
    ...
    // дней прошло, видимо с начала продаж
    ...
    /// а что, кстати полезная фишка - правда непонятно зачем она нужна
